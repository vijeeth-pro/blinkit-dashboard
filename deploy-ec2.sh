...
echo "📌 Strategy: Containerd Storage Reset & Host Port Collision Repair"
echo "================================================================="

# 1. Stop native PostgreSQL host service if running on port 5432
echo "🔌 Stopping any native host PostgreSQL services to prevent port 5432 collision..."
sudo systemctl stop postgresql 2>/dev/null || true

# 2. Force removal of swap file if disk space is dangerously low
...
# 3. Deep system cleanup: purge old kernels, apt archives, and system logs
...
# 4. Reset broken Docker containerd snapshots if disk space is below 1500MB
AVAILABLE_MB=$(df / --output=avail | tail -n 1 | awk '{print int($1/1024)}')
...
# 5. Re-check final available disk space
AVAILABLE_MB=$(df / --output=avail | tail -n 1 | awk '{print int($1/1024)}')
echo "💾 Reclaimed Available Disk Space: ${AVAILABLE_MB} MB"

# 6. Update system package lists ONLY
echo "📦 Updating system package lists..."
sudo apt-get update -y

# 7. Install Docker Engine if missing
...
# 8. Ensure Docker Compose binary is installed and executable
...
# 9. Pull base images sequentially to save temporary layer extraction space
echo "📥 Pulling base Docker images..."
sudo docker pull postgres:16-alpine
sudo docker pull node:22-alpine
sudo docker pull nginx:alpine

# 10. Build & Launch Containers (PostgreSQL, Express API, Nginx SPA)
echo "🏗️ Building and launching Docker containers..."
$DC_CMD down || true
$DC_CMD up -d --build

# 11. Wait for Fresh PostgreSQL Container Health Check
...
# 12. Execute Fresh DB Schema Creation & Dataset Mapping
...
# 13. Clean up unused build cache & temporary files
